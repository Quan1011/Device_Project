...
//     const fetchData = async () => {
//       try {
//         const response = await axios.get("http://localhost:3000/api/device-tree"); // Gọi API từ server
//         setTreeData(response.data); // Cập nhật dữ liệu cây từ phản hồi API
//       } catch (error) {
//         console.error("Error fetching data from server:", error);
//       }
//     };

//     fetchData(); // Thực thi hàm gọi API
//   }, []);
...
export default function CustomTreeView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:3000/api/devices-tree"); // Gọi API từ server
        setTreeData(response.data); // Cập nhật dữ liệu cây từ phản hồi API
      } catch (error) {
        console.error("Error fetching data from server:", error);
      }
  ...
  const handleSelect = async (event, nodeId) => {
    ...
    if (selectedNode) {
      ...
      if (selectedNode.isFolder) {
        // Gọi API để lấy thông tin chi tiết của các thành phần con
        try {
